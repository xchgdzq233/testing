...
namespace testingForms
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //TreeNode asNode = new TreeNode("Amended Sections");
            //for (int i = 0; i < 3; i++)
            //{
            //    asNode.Nodes[i].Name = i.ToString();
            //}
            //rootNode.Nodes.Add(asNode);
            //treeView1.Nodes.Add(rootNode);

            //
            BindingList<Students> list = new BindingList<Students>();
            Students a = new Students { name = "ABC", ID = "123", classID = "1" };
            list.Add(a);
            a = new Students { name = "BCD", ID = "234", classID = "2" };
            list.Add(a);
            a = new Students { name = "DEF", ID = "345", classID = "3" };
            list.Add(a);
        ...
        {
            Thread t = new Thread(new ThreadStart(() => randomNum()));
            setRunning(true);
            t.Start();
        ...
        delegate void delSetLable(string text);

        private void SetLabel(string text)
        {
            if (InvokeRequired)
            {
                this.Invoke(new delSetLable(SetLabel), new object[] { text });
            }
            else
            {
                label1.Text = text;
