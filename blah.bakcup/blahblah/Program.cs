...
namespace blahblah
{
    public class Program
    {
        ...
        static Boolean loadTree = false, loadRule = true, validateSchema = false;
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(String.Format("Found {0} files. Start loading data...\n", files.Length));
            ...
            Console.WriteLine(String.Format("Data loading complete. {0} of files failed.\n", failed));
            if (failed > 0)
                writeToLog(String.Format("Data loading complete. {0} of files failed.\n", failed), null, "INFO");
        ...
        static void LoadChildTree(XmlNode parentEl, NodeClass parentTree)
        {
            foreach (XmlNode currentEl in parentEl)
            {
                ...
                if (Object.ReferenceEquals(currentNodeTreePair, null))
                {
                    ...
                    //old tree
                    currentNodeTreePair.appearCount++;
        ...
        static void PrintSchema()
        {
            ...
            foreach (NodeClass currentNode in resultRule)
            {
                ...
                foreach (NodeAppearPair childNodeAppearPair in currentNode.childNodes)
                {
                    ...
                    //min/maxOccurs
                    if (childNodeAppearPair.appearCount < currentNode.appearCount)
                        currentSchemaEl.MinOccurs = 0;
